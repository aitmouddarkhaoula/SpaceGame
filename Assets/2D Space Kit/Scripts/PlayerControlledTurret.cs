...
public class PlayerControlledTurret : MonoBehaviour {
	...
	void Start () {
		 InvokeRepeating(nameof(Shooting), 0, 0.1f);
	...
	{
        if (barrel_hardpoints != null && GameManager.Instance.InGame)
        {
            GameObject bullet = (GameObject)Instantiate(weapon_prefab, barrel_hardpoints[barrel_index].transform.position, Quaternion.identity);
